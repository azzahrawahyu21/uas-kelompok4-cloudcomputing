    ...
    <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
        <div>
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item active text-muted" aria-current="page">Edit Pejabat</li>
            </ol>
        </div>
    </div>

    <form action="{{ route('pejabat.update', [$pejabat->id_pejabat, $id_sub]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <input type="text" name="nama_pejabat" class="form-control w-100" value="{{ old('nama_pejabat', $pejabat->nama_pejabat) }}" required>
        </div>

        <div class="mb-3">
            <label for="deskripsi" class="form-label fw-semibold">Deskripsi</label>
            <textarea name="deskripsi" id="deskripsi" rows="5" class="form-control w-100">{{ old('deskripsi', $pejabat->deskripsi) }}</textarea>
        </div>

        <div class="mb-3">
            <label for="foto" class="form-label fw-semibold">File atau Gambar</label>
            <div class="input-group w-100">
                <input type="hidden" name="foto_url" id="foto_url" value="{{ asset('ufiles/' . $pejabat->foto) }}">
                <input type="text" name="foto" id="foto" class="form-control" placeholder="Nama file (opsional)" value="{{ old('foto', $pejabat->foto) }}">
                <button type="button" class="btn btn-secondary" id="btnBrowse">Pilih dari ElFinder</button>
            </div>
            <div class="mt-3">
                @if($pejabat->foto)
                    <img id="previewImage" src="{{ asset('ufiles/' . $pejabat->foto) }}" alt="Preview" style="max-width: 200px; border: 1px solid #ccc; padding: 5px; border-radius: 8px;">
                @else
